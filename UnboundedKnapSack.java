...
{ 
    ...
    static int helper(int W, int wt[], int val[], int n,int [][] dp){
         ...
         if(wt[n-1]<=W){
            ...
            // the difference is here , we consider again the instance of a item if it si already processed also
            
             return dp[n][W]= Math.max(val[n-1]+helper(W-wt[n-1],wt,val,n,dp),helper(W,wt,val,n-1,dp));
         }
         else{
